...
describe("weatherMethods", function()  {
    describe("owmGetWeatherReq", async function()  {
        ...
        const axiosStub = sinon.stub(axios, 'get').returns(stub);
        const parseRespondeStub = sinon.stub(weatherMethods, 'parseResponse')
            

        const result = await weatherMethods.owmGetWeatherReq(cities, apiKey);
